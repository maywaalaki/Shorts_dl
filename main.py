...
def webhook():
    ...
    if success and video_url:
        # Waxaan u diraynaa shaqada loop-ka bot-ka uu ku jiro
    ...
    return jsonify({"status": "failed or no url"}), 400
...
async def handler(event):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            await msg.edit("❌ Cilad Engine-ka ah. Hubi in HF uu shaqaynayo.")
    except Exception as e:
        await msg.edit(f"❌ Khalad: Server-ka Engine-ka lama heli karo.")
...
if __name__ == '__main__':
    # Flask thread-ka ku dar
    threading.Thread(target=run_web, daemon=True).start()
    # Bot-ka ku socodsii loop-ka weyn
