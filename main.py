...
def webhook():
    ...
    if success and video_url and main_loop:
        asyncio.run_coroutine_threadsafe(
            handle_upload(chat_id, video_url, title), 
            main_loop
        )
        return jsonify({"status": "processing_upload"}), 200
    return jsonify({"status": "failed"}), 400

async def handle_upload(chat_id, video_url, title):
